...
describe('UrlsService', () => {
  ...
  describe('createShortUrl', () => {
    it('should throw BadRequestException for reserved aliases', async () => {
      await expect(
        service.createShortUrl('http://example.com', 1, 'admin'),
    ...
    it('should throw ConflictException if alias already exists', async () => {
      ...
      await expect(
        service.createShortUrl('http://example.com', 1, 'taken'),
  ...
  describe('getOriginalUrl', () => {
    ...
    it('should throw NotFoundException if isDeleted is true', async () => {
      ...
      await expect(service.getOriginalUrl('deleted')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateUrl', () => {
    ...
    it('should throw NotFoundException if url does not exist', async () => {
      prismaService.url.findUnique.mockResolvedValue(null);
      await expect(
        service.updateUrl(1, 'http://new.com', 10, Role.USER),
    ...
    it('should throw ForbiddenException if user is not the owner', async () => {
      ...
      await expect(
        service.updateUrl(1, 'http://new.com', 99, Role.USER),
